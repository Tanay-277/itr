...
export const AvatarDropZone: React.FC<AvatarDropZoneProps> = ({
  ...
  const [currentImage, setCurrentImage] = useState<string>(`https://avatar.vercel.sh/${id}`);
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      const progressInterval = setInterval(() => {
        setUploadProgress((prevProgress) => (prevProgress >= 95 ? prevProgress : prevProgress + 5));
      }, 500);
      ...
      try {
        ...
        const response = await uploadSubmitAvatarToCloudinary(formData, id!.toString(), "image");

        if (setFormElements && response.data) {
          setFormElements((prevElements) =>
            prevElements.map((element) =>
              element.id === id ? { ...element, value: response.data.secure_url! } : element,
            ),
          );
        }
