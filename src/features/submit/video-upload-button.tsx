...
export const VideoUploadButton: React.FC<VideoUploadFieldProps> = ({
      ...
      if (error instanceof DOMException) {
        switch (error.name) {
          case "NotAllowedError":
            errorToast(`${type} access was denied. Please allow access in your browser settings.`);
            break;
          case "NotFoundError":
            errorToast(`No ${type} device found.`);
            break;
          default:
            errorToast(`Error accessing ${type}. Please check your device settings.`);
        }
      }
      return false;
    }
  ...
  const handleVideoUpload = useCallback(
    async (mediaBlobUrl: string) => {
      ...
      try {
        ...
        const uploadResponse = await uploadToCloudinary(formData, id.toString(), "video");
  ...
  if (showPermissionPrompt) {
    return (
      <Card className="w-full">
        <CardContent className={cn("m-1 rounded-lg border border-dashed border-[#7c533a]/50 p-6")}>
              ...
              <div className="flex items-center justify-between rounded-lg bg-secondary p-3">
                ...
                <div>
                  {permissionState.camera ? (
                    ...
                    <Button onClick={handleCameraPermissionRequest} variant="default" size="sm">
              ...
              <div className="flex items-center justify-between rounded-lg bg-secondary p-3">
                ...
                <div>
                  {permissionState.microphone ? (
                    ...
                    <Button onClick={handleMicrophonePermissionRequest} variant="default" size="sm">
  ...
  return (
    <Card className="w-full">
      <CardContent
        className={cn("m-1 cursor-pointer rounded-lg border border-dashed border-[#7c533a]/50 p-6")}
      >
        <ReactMediaRecorder
          video
          audio
              ...
              <div className="flex flex-col items-center justify-center">
                ...
                {videoUrl && (
                  <div className="mt-4 flex w-full flex-col items-center">
                    <video src={videoUrl} controls className="mb-4 w-full max-w-xs rounded-lg" />
                  </div>
                )}
